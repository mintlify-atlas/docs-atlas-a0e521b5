---
title: "Admin User Management in Eventify"
...
<Warning>
  Deletion is immediate and irreversible through the UI. The `users` table schema includes a `softDeletes` column, but `UserController::destroy` calls `$user->delete()` which, without the `SoftDeletes` trait on the `User` model, performs a hard delete. Verify the intended behaviour before removing an account.
</Warning>
