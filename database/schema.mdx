...
<Note>
  All tables except `roles` and `categories` that carry a `deleted_at` column use Laravel's **SoftDeletes** trait. Soft-deleted rows are excluded from standard queries but remain in the database and can be restored at any time.
</Note>
